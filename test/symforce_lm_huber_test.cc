...
TEMPLATE_TEST_CASE("Converges for a linear problem in one iteration", "[levenberg_marquardt]",
    ...
    auto residual_func = [&](const sym:: Values<Scalar>& values, 
        ...
        //Apply Huber Norm to the residuals
        const Scalar huber_threshold = 1000.0;
        for (int i = 0; i < linearization.residual.size(); i++) {
            if (std::abs(linearization.residual(i)) > huber_threshold) {
                linearization.residual(i) = std::sqrt(huber_threshold / linearization.residual(i)) * linearization.residual(i);
            }
        }
